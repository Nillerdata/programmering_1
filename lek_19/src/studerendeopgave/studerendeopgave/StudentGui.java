package studerendeopgave.studerendeopgave;
...
//
		...
		btnBirthday.setOnAction(event-> this.studerende.increaseAge());

	}

	private void addAction() {
		if (studerende == null) {
			int alder = Integer.parseInt(txage.getText());
			studerende = new Student(txfName.getText().trim(),alder, chkActive.isSelected());
			clearFields();
	...
	private void saveAction() {
		if (studerende != null) {
			...
			studerende.setAge(Integer.parseInt(txage.getText()));
			clearFields();
			txAInf.setText(getDescription());
			btnSave.setDisable(true);
			btnDelete.setDisable(true);
